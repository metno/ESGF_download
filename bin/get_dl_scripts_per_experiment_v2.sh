...
if [[ $# -lt 1 ]] 
	then echo "usage: ${0} <experiment> <dl directory>"
	echo example: ${0} piControl ../dl_scripts/ 
	exit
fi


scriptdir=`realpath ${2}`
...
runfile="./runfile_${1}_${logdate}_${UUID}_${HOSTNAME}.run"
rm -f "${runfile}"
logfile="${logdir}/runfile_${1}_${logdate}_${UUID}_${HOSTNAME}.log"
DryRunFlag=0
